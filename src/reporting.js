...
function deriveIssues(result){
  ...
  for(const t of result.thresholds.filter(x=>!x.ok)){
    ...
    issues.push({type:'threshold',status:'FAIL',severity:'HIGH',title:t.metric,detail:`Threshold breached: ${t.expression}`,actual:'',sla:''});
  }
  return issues;
}
...
export function clientHtmlReport(data, meta={}){
  ...
  const coverageHtml=r.coverageMatrix.map(c=>{
